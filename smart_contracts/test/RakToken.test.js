...
describe("RakToken Unit Test", function () {
  ...
  beforeEach(async function () {
    ...
    ourToken = await ethers.getContract("RakToken", deployer);
  });
  ...
  describe("constructor", () => {
    ...
    it("initializes the token with the correct name and symbol ", async () => {
      ...
      assert.equal(name, "RakToken");

      const symbol = (await ourToken.symbol()).toString();
      assert.equal(symbol, "RAK");
    });
  });
  describe("minting", () => {
  ...
  describe("transfers", () => {
    ...
    describe("allowances", () => {
      ...
      beforeEach(async () => {
        playerToken = await ethers.getContract("RakToken", user1);
      });
      it("Should approve other address to spend token", async () => {
        const tokensToSpend = ethers.utils.parseEther("5");
        await ourToken.approve(user1, tokensToSpend);
        const ourToken1 = await ethers.getContract("RakToken", user1);
