...
describe("KarToken Unit Test", function () {
  ...
  beforeEach(async function () {
    ...
    ourToken = await ethers.getContract("KarToken", deployer);
  });
  ...
  describe("constructor", () => {
    ...
    it("initializes the token with the correct name and symbol ", async () => {
      ...
      assert.equal(name, "KarToken");

      const symbol = (await ourToken.symbol()).toString();
      assert.equal(symbol, "KAR");
    });
  });
  describe("minting", () => {
  ...
  describe("transfers", () => {
    ...
    describe("allowances", () => {
      ...
      beforeEach(async () => {
        playerToken = await ethers.getContract("KarToken", user1);
      });
      it("Should approve other address to spend token", async () => {
        const tokensToSpend = ethers.utils.parseEther("5");
        await ourToken.approve(user1, tokensToSpend);
        const ourToken1 = await ethers.getContract("KarToken", user1);
